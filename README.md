...
3. **別ルートの自動評価** — 旅館・ホテル営業がNGなら、簡易宿所・住宅宿泊事業（民泊）
   で成立するかを同じ物件条件で評価
4. **自治体条例の自動調査** — 検索できるLLM（**既定は Gemini**＝Google検索グラウンディング、
   Claude も選択可）が自治体の例規集・審査基準を実際に開いて確認し、原文引用と
   公的ドメインのURLが取れたものだけを判定に使う
...
6. **朝会1枚の出力**（「🗣️ 朝会1枚」タブ） — 毎日15分の朝会で
   ...
   **築古（築30年超）は「売出価格÷土地値」で価格の合否を判定**する（建物価値が0に近づき、
   土地をいくらで買うかに収斂するため）。判定閾値の正本は `config/screening_rules.yaml`
...
```
  ...
  │ 全国共通の法令   │          │ Gemini/Claude + web検索で調査 │
...
### Gemini API（必須・これ1本で足りる）
- https://aistudio.google.com/apikey で取得（無料枠あり）
- `.env` の `GEMINI_API_KEY` にセット
- 使うところ：①書類（重説・検査済証PDF）の自動抽出 ②自治体条例のweb検索調査
  ③用途地域の候補調べ ④結果チャット・追加情報の解釈

### Anthropic Claude API（任意）
- https://console.anthropic.com/ で取得し `ANTHROPIC_API_KEY` にセット
- `LLM_PROVIDER=claude` / `LEGAL_RESEARCH_PROVIDER=claude` にすると Claude を使う
...
- ⚠️ **ここはLLMで代用しない**（用途地域を誤ると判定が反転する）。キーが無い間は
  重説・レインズの記載を手入力するか、`--zoning-lookup` で Gemini に候補を出させて人が確認する
- **土地単価もこのキーで引く**（`XPT002` 地価公示・地価調査の近傍標準地 → `api/land_price.py`）。
  築古は「売出価格 ÷ 土地値」で合否が決まるので、ここが効く
