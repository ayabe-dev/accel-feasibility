...
def compute(report: FeasibilityReport, overrides: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # 土地値。築古（耐用超過）は建物価値が0になるので、原価法＝実質「土地値」になる。
    # 土地坪単価はエリア既定（area_tiers）の代表値だが、路線価・地価公示・取引事例で
    # 実勢が分かっているなら手入力（land_price_per_tsubo_man）が優先される。
    # 土地坪単価は3段の優先順位で決める：
    #   ① 手入力（人が実勢を知っている）
    #   ② 地価公示・地価調査の近傍標準地（公的な実データ・出所つき）
    #   ③ エリア4区分の代表値（最後の砦。同じ23区内が一律なので粗い）
    land_unit_override = o.get("land_price_per_tsubo_man")
    land_points: List[Dict[str, Any]] = []
    if land_unit_override:
        land_tier = {"min": land_unit_override, "mid": land_unit_override,
                     "max": land_unit_override}
        land_unit_source = "手入力（路線価・地価公示・取引事例など実勢）"
    else:
        official = None
        if str(os.getenv("LAND_PRICE_LOOKUP", "true")).lower() not in ("false", "0", "no"):
            try:
                from api import land_price as _land_price

                official = _land_price.lookup(
                    getattr(geo, "latitude", None),
                    getattr(geo, "longitude", None),
                    zoning_code=getattr(geo, "zoning_code", None),
                )
            except Exception:  # noqa: BLE001 - 取得失敗で試算を止めない
                official = None
        if official:
            land_tier = official["per_tsubo_man"]
            land_unit_source = official["source"]
            land_points = official.get("used_points") or []
            if not official.get("matched_use_category"):
                warnings.append(
                    "土地単価：用途区分が一致する標準地が近くに無く、近傍点で代用しています"
                    "（実勢が分かるなら土地坪単価を手入力してください）"
                )
        else:
            land_tier = tier["land_price_per_tsubo_man"]
            land_unit_source = f"エリア既定（{tier['label']}）の代表値。実勢との突合が必要"
    land_tsubo = m2_to_tsubo(land_area) if land_area else 0.0
    land_value = {k: land_tier[k] * land_tsubo for k in ("min", "mid", "max")} if land_area else None
    land_value_mid = land_value["mid"] if land_value else 0.0
    ...
    land_block = {
        ...
        "land_tsubo": round(land_tsubo, 1) if land_area else None,
        "unit_price_per_tsubo_man": dict(land_tier),
        "unit_price_source": land_unit_source,
        "unit_price_is_override": bool(land_unit_override),
        "unit_price_is_official": bool(land_points),
        "official_points": land_points,
        "land_value_man": ({k: round(v, 1) for k, v in land_value.items()} if land_value else None),
        "building_recost_man": round(build_recost, 1),
        "building_value_man": round(building_value, 1),
        "depreciation_pct": round(_clamp(age / useful, 0, 1) * 100, 1),
        "building_is_worthless": building_value <= 0.01,
    ...
    valuation = {
        ...
        # 土地値ベースの見方（築古では原価法≒土地値になるため、こちらが実務の主軸）
        "land_value_mid_man": round(land_value_mid, 1) if land_value else None,
        "price_to_land_ratio": None,      # 売出価格 ÷ 土地値（mid）
        "excess_over_land_man": None,     # 土地値を超えて払う分（＝上物・のれんへの支払い）
    }
    if asking:
        ...
        if land_area:
            # 上物をゼロ評価したときの土地坪単価。実勢の坪単価と直接比べられる数字
            valuation["asking_land_per_tsubo_man"] = round(asking / m2_to_tsubo(land_area), 1)
            valuation["tier_land_per_tsubo_man"] = land_tier
            if land_value_mid > 0:
                valuation["price_to_land_ratio"] = round(asking / land_value_mid, 2)
                valuation["excess_over_land_man"] = round(asking - land_value_mid, 1)
