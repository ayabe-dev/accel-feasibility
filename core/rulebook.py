...
def _s_pipeline() -> str:
    return "\n".join(
        [
            ...
            "          │        core/legal_research.py（Gemini/Claude の検索調査・検証済みのみ採用）",
            "          │",
            ...
            "条例の探索と引用は**LLMで**（既定は Gemini＝Google検索）。LLMの所見が判定を動かせるのは"
...
def _s_evidence() -> str:
    ...
    L.append(
        ...
        "自治体条例は `core/legal_research.py`（既定 Gemini・Claude も可）が調査し、"
...
def _s_screening() -> str:
    ...
    L = [
        ...
        "**価格をどちらの基準で見るか**",
        "",
        f"築年数が `land.prefer_land_basis_if_age_over`（現在 "
        f"{(d.get('land') or {}).get('prefer_land_basis_if_age_over', '—')}年）を超える物件は、"
        "価格の合否を**土地値比（売出価格÷土地値）**で判定し、"
        "収益還元＋原価法の適正価格レンジ比は参考に落とす。"
        "築古は建物価値が0に近づき「土地をいくらで買うか」に収斂するため。",
        "",
        "> ⚠️ **土地値の精度がボトルネック**。土地坪単価は `revenue_estimates.yaml` の"
        "エリア4区分の代表値（都心500／23区300／京都280／その他120 万円/坪）で、"
        "**路線価・地価公示・取引価格情報のAPIは未接続**（`api/gis_client.py` は都市計画系の"
        "XKT002/014/001/023/024/006/007 のみ）。実勢が分かる場合は②収益化タブの"
        "「土地坪単価」に手入力すると、その値が優先される。",
    ]
    return "\n".join(L)
