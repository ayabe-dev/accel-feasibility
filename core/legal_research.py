"""自治体条例・手引きを LLM（web検索つき）で一次情報から調べる層.

検索できるLLMをプロバイダとして差し替えられる（既定は Gemini＝Google検索
グラウンディング、次に Claude＝server-side web search）。どちらを使っても
検証ガード（原文引用＋公的ドメインURL）は同じで、満たさない所見は判定を動かさない。
...
## なぜこの設計か
  ...
  一方で LLM に丸ごと判定させると数値基準を誤る。よって：
      ...
      条例の探索と引用   → Gemini（Google検索）または Claude + web検索（カバレッジ）
...
class _RetryWithOtherModel(RuntimeError):
    """モデルを変えれば直る見込みの失敗（空応答・JSON崩れ）."""
...
# モデル名の解決（廃止への自己修復を含む）は core/gemini_models に集約している。
# 互換のため名前は残すが、実体は共通の候補チェーン。
from . import gemini_models  # noqa: E402

GEMINI_MODEL_FALLBACKS = gemini_models.candidate_chain(purpose="quality")
...
_NO_CREDENTIAL_MESSAGE = (
    "検索できるLLMの資格情報が見つからないため自治体調査をスキップしました。"
    "**GEMINI_API_KEY**（推奨）または ANTHROPIC_API_KEY を設定してください。"
...
def active_provider() -> Optional[str]:
    """実際に使えるプロバイダ名を返す（使えなければ None）.

    LEGAL_RESEARCH_PROVIDER=gemini|claude で固定できる。既定は auto＝
    **Gemini を優先**（キーが1本で済むため）、無ければ Claude。
    """
    want = (os.getenv("LEGAL_RESEARCH_PROVIDER") or "auto").strip().lower()
    if want == "gemini":
        return "gemini" if _gemini_available() else None
    if want == "claude":
        return "claude" if _claude_available() else None
...
def research_municipality(
    ...
    if provider == "claude":
        ...
        model = os.getenv("LEGAL_RESEARCH_GEMINI_MODEL", GEMINI_MODEL_FALLBACKS[0])
...
確認できなかったことは unresolved に入れてください。"""
    ...
    try:
        if provider == "gemini":
            raw, model = _run_research_gemini(model, user_prompt)
        else:
            tools = [
                {
        ...
        msg = str(e)
        low = msg.lower()
        if "authentication" in low or "api_key" in low or "api key" in low:
            err = _NO_CREDENTIAL_MESSAGE
        else:
            err = f"自治体調査でエラーが発生しました（{provider}）：{e}"
        return ResearchResult(
            municipality_key=municipality_key,
            municipality_name=municipality_name,
            error=err,
...
def _run_research_gemini(
    model: Optional[str], user_prompt: str
) -> "tuple[Optional[Dict[str, Any]], str]":
    """Gemini（Google検索グラウンディング）で調べてJSONにする.

    2段構えにしている理由: Gemini は検索ツールと構造化出力（response_schema）を
    同時に使えないため、①検索して根拠を集める ②その結果だけを材料にJSONへ整形
    に分ける。②では検索させない（新しい事実を足させない）。

    モデル名の解決・廃止時の自己修復は core/gemini_models に任せる。
    戻り値は (JSON, 実際に使えたモデル名)。
    """
    ...
    search_prompt = (
        user_prompt
        + "\n\n## 出典の書き方（厳守）\n"
        "- URLは**実際に開いた一次情報のURL**をそのまま書く"
        "（例規集・自治体サイト・省庁サイト。`go.jp` / `lg.jp` 等）。\n"
        "- 検索エンジンの中継URL・短縮URL・キャッシュURLは書かない。\n"
        "- 引用は**条文・手引きの原文**をそのまま抜く（要約を引用に入れない）。\n"
        "- 確認できなかったことは推測で埋めず、確認できなかったと書く。"
    )
    ...
    def _run(model_name: str) -> Dict[str, Any]:
        ...
        if not notes:
            raise _RetryWithOtherModel("調査応答が空でした")

        # ② 集めた根拠だけを材料にJSONへ整形（検索はさせない）
        structured = client.models.generate_content(
            model=model_name,
            contents=(
                "次の調査メモを、指定のJSONスキーマに厳密に従うJSONへ整形してください。\n"
                "**メモに書かれていない事実を足さないでください。**"
                "URLと引用はメモにあるものだけを使い、無いものは空文字にしてください。\n\n"
                "=== 調査メモ ===\n" + notes
            ),
            config=genai_types.GenerateContentConfig(
                system_instruction=_SYSTEM_PROMPT + _json_fallback_instruction(),
                response_mime_type="application/json",
                temperature=0.0,
            ),
        )
        parsed = _loads_lenient(getattr(structured, "text", "") or "")
        if parsed is None:
            raise _RetryWithOtherModel("応答をJSONとして解釈できませんでした")
        return parsed

    parsed, used_model, _tried = gemini_models.call_with_fallback(
        client,
        _run,
        purpose="quality",
        primary=model or None,
        also_retry=lambda e: isinstance(e, _RetryWithOtherModel),
    )
